...
CHECK_IMDB_ID = True # Useful for clearly identifying the movie in case the title at letterboxd is different from imdb
...
def check_authentication():
    if os.path.isfile('auth.json'):
        with open('auth.json') as auth_file:
            ...
            if expired:
                print('Authentication expired, trying to re-authenticate...')
...
def get_data_letterboxd(filename,diary=True, ratings=False, rating_name=""):
    ...
    with open(filename, 'r', encoding='utf-8') as csvfile:
        ...
        for row in letterboxd_data:
            ...
            if diary:
                ...
                ratings_data = csv.reader(open(rating_name, 'r'), delimiter=',')
                for line in ratings_data:
                    if line[3] == row[3]:
                        data.append([row[1],row[2],'released',imdbid,line[4]])
                        print([row[1],row[2],'released',imdbid,line[4]])
                        break
            else:
...
def send_data(movie_data, auth_token, ratings):
...
def get_trakt_data(auth_token):
    ...
    req = urllib2.Request('https://api.trakt.tv/sync/watched/movies', headers=headers)

    f = urllib2.urlopen(req)
    response = f.read()
    f.close()
    data = json.loads(response)
    with open("prova2.json", "w", encoding="utf-8") as a:
        json.dump(data, a)
    ...
    for i, movie in enumerate(data):
        ...
        if imdbid not in watched_ids:
            watched_ids[imdbid] = [i]
        else:
            watched_ids[imdbid].append(i)

    return data, watched_ids

def get_release_date(imdbid, auth_token):
    headers = {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer ' + auth_token,
    'trakt-api-version': '2',
    ...
    req = urllib2.Request('https://api.trakt.tv/movies/' + imdbid + '/releases/us', headers=headers)
    f = urllib2.urlopen(req)
    response = f.read()
    f.close()
    ...
    data = json.loads(response)
    for rel in data:
        if rel["release_type"] == "theatrical":
            return rel["release_date"]
    
    return data[0]["release_date"]

def is_duplicate(trakt_data, movie_data, watched, auth_token):
    date_played = movie_data[2]
    imdbid = movie_data[3]

    if imdbid not in watched:
        return False
    
    for i in watched[imdbid]:
        movie = trakt_data[i]
        if date_played == "released":
            #release_date = get_release_date(imdbid, auth_token)
            #if release_date == movie["last_watched_at"][0:10]:
            return True
        elif date_played == movie["last_watched_at"][0:10]:
            return True
...
if __name__ == "__main__":
    use_diary_file = True
    use_ratings_file = False
    ratings_filename = ""
    try:
        args, letterboxd_file = getopt.gnu_getopt(sys.argv[1:], 'h:w:r',[
            'help','watched', 'ratings'])
    except:
    ...
    for opt, arg in args:
        if opt in ['--help', '-h']:
            ...
            use_ratings_file = True
            ratings_filename = letterboxd_file[1]
        else:
    ...
    data = get_data_letterboxd(letterboxd_file[0],use_diary_file, use_ratings_file, ratings_filename)
    trakt_data, watched = get_trakt_data(token)

    print(str(len(data)) + ' movies in file.')
    movie_data = [];
    movie_data_no_ratings = []
    skipped = [];
    duplicated = []
    ...
    for line in data:
        ...
        elif is_duplicate(trakt_data, line, watched, token):
            duplicated.append(title)
        elif rating == None:
            movie_data_no_ratings.append({'title':title,'year':int(year),'watched_at': date_played,'rating':rating,'rated_at':date_played,'ids':{'imdb':imdbid} })
        else:
        ...
        if len(movie_data_no_ratings) >= 100:
            print('Importing next 100 movies...')
            send_data(movie_data_no_ratings,token,False)
            movie_data_no_ratings = []

        if len(movie_data) >= 100:
            print('Importing next 100 movies...')
            send_data(movie_data,token,True)
            movie_data = []

    if len(movie_data) > 0:
        send_data(movie_data,token,True)

    if len(movie_data_no_ratings) > 0:
        send_data(movie_data_no_ratings,token,False)
